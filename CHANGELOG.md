...
| [0.116.0] | 2026-03-22 | Terminology cleanup — replaced all user-facing "IMPL" labels with "plan": `CreateFromImplsPanel` title/filter/empty states, `ProgramBoard` progress counters + "Select at least 2" hint + completion banner, `ImportPanel` auto-discover label + column header, `NotificationSettings` event label |
| [0.115.0] | 2026-03-22 | Program-IMPL navigation — graph is default program view (pill order swapped); program-member IMPLs now visible in home screen pipeline (removed standalone filter); review page shows "← program-name · Tier N" breadcrumb linking back to program graph; `UnifiedProgramsView` accepts `initialProgramSlug` for cross-view navigation |
| [0.114.0] | 2026-03-22 | Program context frame + IMPL completion — IMPL dep graph shows dashed program context border with program name and tier when IMPL belongs to a program; `IMPLDocResponse` extended with `program_slug/title/tier/tiers_total` fields populated from `implProgramCache`; program graph IMPL containers show green border + bottom-right checkmark for completed IMPLs; status comparison handles `SAW:COMPLETE` (case-insensitive); agent nodes changed from circles to rounded rects matching DependencyGraphPanel style; wave row backgrounds use full-width proportions |
...
| [0.109.0] | 2026-03-21 | High-contrast mode theme-aware boost — replaces hardcoded black/white overrides with computed lightness adjustments; reads current theme's actual HSL values via `getComputedStyle` and pushes them toward extremes (hue preserved); `MutationObserver` re-injects when dark mode or theme class changes; static CSS blocks removed from `index.css` |
| [0.108.0] | 2026-03-21 | High-contrast mode settings integration — contrast preference now saved in `appearance` config alongside theme; `SettingsScreen` loads full `appearance` object (fixes contrast being wiped on settings save); theme onChange spreads existing appearance fields; high contrast checkbox added to Appearance section |
| [0.107.0] | 2026-03-21 | High-contrast mode — `.high-contrast` CSS class toggled on `<html>`; `useContrast` hook mirrors `useDarkMode` pattern; vars injected as unlayered `<style>` tag (beats theme vars in cascade); only readability vars overridden (background, foreground, border, muted, card, popover — theme colors preserved); `HighContrastToggle` half-circle icon button in AppHeader next to dark mode toggle; `contrast` field added to `SAWConfig.appearance` (TS) and `AppearConfig` (Go) |
| [0.106.0] | 2026-03-21 | UI fixes — `ImplList` delete X button z-index fix (`z-[2]`) restores per-row delete on active IMPLs (matches completed section); `hover:bg-muted/40` highlight awareness on IMPL rows; `useResizableDivider` exposes `isDragging` state; `AppLayout` suppresses `transition-[width]` on sidebar during drag and on right panel during right-drag — eliminates resize lag on both left sidebar and WaveBoard |
| [0.105.0] | 2026-03-21 | Webapp API feature parity — `pkg/api/interview_handlers.go` SSE-streamed interview flow (start/events/answer/cancel endpoints); `pkg/api/validation_handlers.go` integration + wiring validation endpoints; `pkg/api/import_handlers.go` bulk IMPL import with auto-discovery and tier assignment; `pkg/api/server.go` registers 7 new routes + `interviewRuns sync.Map`; `InterviewLauncher.tsx` 3-view component (launch form → active session → completion) with 6-phase progress bar; `ValidationPanel.tsx` gap list with severity badges; `ImportPanel.tsx` discover mode + tier assignment table; `apiClient.ts` extended with `interview`, `validateIntegration`, `validateWiring`, `importImpls` namespaces |
| [0.104.0] | 2026-03-21 | Model role consolidation — `web/src/types/models.ts` new canonical module with `ModelRole` union type, `MODEL_ROLES` ordered array, `defaultModels` record; `AppContext.tsx` 7 individual model `useState` hooks replaced with single `Record<ModelRole, string>`; `AppHeader.tsx` local `ModelRole` type + `ROLES` constant removed, now imports from shared module; `types.ts` doc comment added pointing to `ModelRole` as authoritative type |
| [0.103.0] | 2026-03-21 | Centralize status helpers — `web/src/lib/statusColors.ts` with 8 exported functions (`getStatusBadgeClasses`, `getStatusHoverClass`, `getStatusLabel`, `getStatusBorderColor`, `getStatusGlowStyle`, `getNodeFillColors`, `getProgramStateDotClass`, `getSuitabilityBadgeClasses`) + 68 tests; `pkg/api/helpers.go` shared HTTP helpers (`respondJSON`, `respondError`, `decodeJSON`) + 10 tests; AgentCard, PipelineRow, ProgramBoard, ProgramDependencyGraph, SuitabilityBadge, StatusBadges migrated to `statusColors.ts`; all 17 API handler files migrated to `respondJSON`/`respondError`/`decodeJSON` |
| [0.102.0] | 2026-03-21 | Persistent collapsed WaveBoard panel — `AppLayout` gains `rightPanelCollapsed`/`onToggleRightPanel`/`rightPanelCollapsedContent` props; collapsed state renders 40px strip with `transition-[width] duration-200`; `ActionButtons` gains `waveBoardExpanded` prop toggling Eye/EyeOff icon and "View WaveBoard"/"Hide WaveBoard" label; `App.tsx` wires `waveBoardCollapsed` state (defaults true, resets on `liveView` activate); Waves icon shown in collapsed strip; WaveBoard header row is fully clickable to collapse (cursor-pointer, hover:bg-muted/50); `LiveRail` idle text updated to "Select an action to begin." |
| [0.101.0] | 2026-03-21 | UI polish batch — `agentColors.ts` → `entityColors.ts` with `getRepoColor`/`getRepoColorWithOpacity` (hash-based deterministic hue); repo color-coded left borders + tinted badges on pipeline rows; multi-select repo filter tabs (toggle independently, All toggles entire set); critic model selector added to top nav; `AppLayout` + `OperationsPanel` smooth `transition-[width]` collapse with click-anywhere-to-expand; `ImplList` animated repo expand/collapse via `grid-template-rows: 0fr→1fr`; selection highlight cascade (snaps to list top, slides to selected row when repo expands); drag resize no longer triggers transition animation |
| [0.100.0] | 2026-03-21 | Create-from-IMPLs GUI + repo mismatch web — `POST /api/impl/analyze-impls` (conflict detection + tier suggestions) + `POST /api/impl/create-from-impls` (PROGRAM manifest generation) endpoints; `CreateFromImplsModal` + `ConflictReport` frontend components; Add-to-Program `+` button on active pipeline rows with hover tooltip; cross-repo mismatch detection in `wave_service.go` redirects agents to correct repo |
| [0.99.0] | 2026-03-21 | Program dependency graph enhancements — vertical layout (tiers as rows, top-to-bottom flow); wave progress bars inside IMPL nodes; clickable nodes; status badges; responsive width; list/graph view toggle in `ProgramBoard` detail view |
| [0.98.0] | 2026-03-21 | Unified navigation — Programs is now the default home view; Pipeline merged into `UnifiedProgramsView` with active/completed sections; `OperationsPanel` (Queue/Daemon/Settings) exposed as collapsible right rail; `SidebarNav` always shows IMPL list with Programs section collapsible at top; `PipelineRow` reused for home page list with program-link badge and tier progress |
| [0.97.0] | 2026-03-20 | UI polish batch — tooltips on all header/panel buttons, lucide-react chevrons (theme-aware, scalable), individual model role buttons in header (planner→scout→scaffold→wave→integration→chat), content-aware panel defaults (scaffolds/contracts/wiring auto-enable), collapsible wave containers in WaveBoard, SSE-reactive interrupted sessions banner (shows running state, clears on complete), disk-authoritative merge/agent state (fixes stale SSE overrides), display-only CriticReviewPanel (auto-fetches on load + SSE refresh), removed manual critic button, sidebar IMPL row bg contrast, removed completed IMPL docs |
| [0.96.0] | 2026-03-19 | `POST /api/impl/{slug}/amend` + `AmendPanel.tsx` (living-impl-docs) — `AmendImplRequest`/`AmendImplResponse` in `pkg/api/amend_handler.go`; `RegisterAmendRoutes` wired into `server.go New()`; 3-tab `AmendPanel.tsx` (add-wave, redirect-agent with wave/agent dropdowns, extend-scope with Scout guidance); `Amend` tab added to `ReviewScreen.tsx` Post-Execution group |
| [0.95.0] | 2026-03-19 | AI code review gate web (ai-code-review-gate) — `StepCodeReview` in pipeline via build-tag bridge (`codereview_bridge.go` / `_stub.go`); `ReviewResultPanel.tsx` component; `CodeReviewCfg` + `AgentConfig.ReviewModel` in `pkg/api/types.go`; config handler + settings UI; route registered in `server.go` |
| [0.94.0] | 2026-03-19 | E34 Replan UI — Replan button (amber, visible when program state is `BLOCKED`), `replanning` state, SSE listeners for `program_replan_complete`/`program_replan_failed` in `ProgramBoard.tsx` |
| [0.93.0] | 2026-03-19 | E35 SSE wiring events — `wave_runner.go` emits `wiring_gap` (one per gap) and `wiring_gaps_summary` SSE events in both monolithic and pipeline finalize paths; non-blocking (logs and continues); follows `stub_report` event naming convention |
| [0.92.0] | 2026-03-19 | E35 web API + UI — `WiringEntry` struct in `pkg/api/types.go`; `Wiring []WiringEntry` field on `IMPLDocResponse`; `implDocResponseFromManifest` populates from `manifest.Wiring`; `WiringPanel.tsx` new component (card grid with declared/verified/gap status badges); registered in `ReviewScreen.tsx` alongside PreMortem, Contracts, and Scaffolds panels |
